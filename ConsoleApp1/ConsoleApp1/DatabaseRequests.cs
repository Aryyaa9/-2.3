...
public static class DatabaseRequests
{
    ...
    public static void EditTask(int id)
    {
        ...
        using (var cmd = new NpgsqlCommand($"UPDATE tasks (name, id_user, description, date_task) VALUES ('{newName}', '{id}', '{newDescription}', '{newDate}')", DatabaseService.GetSqlConnection()))
        {
            int rows = cmd.ExecuteNonQuery();
            if (rows > 0)
            {
    ...
    public static void ViewTasksToday(int id, DateTime today)
    {
        using (var cmd = new NpgsqlCommand($"SELECT * FROM tasks WHERE id_user = '{id}' AND date_task = '{today}'", DatabaseService.GetSqlConnection()))
        {
            ...
            while (reader.Read())
            {
                Console.WriteLine($"Название: {reader.GetString(0)}, Описание: {reader.GetString(1)}, Дата: {reader.GetDateTime(2)}");
            }
        }
    }
